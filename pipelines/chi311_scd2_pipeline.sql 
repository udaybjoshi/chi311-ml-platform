...
--   - Gold tables PARTITIONED BY date for efficient time-range queries
--   - CLUSTER BY on high-cardinality columns (sr_number, ward)
--   - ZORDER BY on frequently filtered columns
--   - Auto-optimization enabled via pipelines.autoOptimize.managed
--   - Predicate pushdown enabled via partition pruning on date
--   - Target file size: 128MB (delta.targetFileSize)
--   - Auto-compaction enabled for small file management
--
-- Pipeline Configuration (set in DLT pipeline settings JSON):
-- {
--   "spark_conf": {
...
--     "spark.sql.adaptive.skewJoin.skewedPartitionThresholdInBytes": "209715200",
...
-- 5. Add spark_conf settings above to pipeline JSON configuration
-- =============================================================================
...
CLUSTER BY (sr_number);  -- Cluster by business key for efficient lookups
...
-- =============================================================================
-- SILVER LAYER: Current State Table (Materialized for Caching)
-- =============================================================================
-- Purpose: Materialized current record state for efficient Gold layer queries
-- Filters SCD2 table for only active records (__END_AT IS NULL)
-- 
-- Optimization: Materialized as LIVE TABLE instead of VIEW to cache results
-- and avoid repeated computation across multiple Gold table dependencies
-- =============================================================================

CREATE LIVE TABLE silver_current_311_requests
COMMENT "Current state of 311 requests (latest version only) - materialized for caching"
...
-- Optimization: Partitioned by date, auto-optimize, target 128MB files
-- Query Pattern: Filter by date for efficient partition pruning
-- -----------------------------------------------------------------------------
CREATE LIVE TABLE gold_daily_aggregates
COMMENT "Daily aggregates by ward and service request type"
PARTITIONED BY (date)
CLUSTER BY (ward, sr_type)
TBLPROPERTIES (
    "quality" = "gold",
    "pipelines.autoOptimize.managed" = "true",
    "delta.autoOptimize.optimizeWrite" = "true",
...
-- Optimization: Partitioned by date, clustered by ward for efficient forecasting queries
-- -----------------------------------------------------------------------------
...
-- Optimization: Partitioned by date, clustered by sr_type
-- -----------------------------------------------------------------------------
